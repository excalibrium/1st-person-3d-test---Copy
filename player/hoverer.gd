...
func _physics_process(_delta):
	if last_hovered in buttons and last_hovered.get_parent().visible == true:
		visible = true
		#print("WOOOO")
	elif Input.is_action_just_pressed("shoot") and last_hovered in buttons:
		visible = false
	if Input.is_action_just_pressed("shoot") and (get_parent().in_menu or get_parent().in_ingame_menu):
		clicked = true
		current_clicked = cursor.pressed_on
	if clicked == false or last_hovered == null:
		#visible = true
		get_mesh().surface_get_material(0).grow_amount = lerp(get_mesh().surface_get_material(0).grow_amount, 0.0005, 0.3)
	if current_hovered != null and last_hovered != null and current_hovered in buttons and last_hovered in buttons and (get_parent().in_menu or get_parent().in_ingame_menu) and current_hovered.get_parent().visible == true: #if hovering over a button
		get_mesh().size = lerp(get_mesh().size, current_hovered.get_parent().find_child("mesh").get_mesh().size / 4.0, 0.4)
		if last_hovered.get_parent().hoveredOn == true and current_hovered != last_hovered and get_mesh().surface_get_material(0).grow_amount >= 0.001:
			get_mesh().surface_get_material(0).grow_amount = 0.03
		if current_clicked != current_hovered:
			get_mesh().surface_get_material(0).grow_amount = 0.0005
		if current_clicked in buttons:
			if current_clicked.get_parent().hoveredOn == true and clicked:
				get_mesh().surface_get_material(0).grow_amount = lerp(get_mesh().surface_get_material(0).grow_amount, 0.03, 0.5)
		if current_clicked == last_hovered and Input.is_action_just_released("shoot"):
			get_mesh().surface_get_material(0).grow_amount = -0.03
